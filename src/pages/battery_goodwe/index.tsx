...
export default function IndexPage() {
  ...
  const fileSizeMapRef = useRef<Record<string, Vector3>>({});
        ...
        (gltf) => {
          ...
          gltf.scene.traverse((child) => {
            if (child instanceof Mesh) {
              ...
              child.name = `${name}_${index}_${child.name}`;
              index++;
            }
          });
          const box = new Box3();
  ...
  const setCarColor = useCallback((index: number) => {
    ...
    // 修改某个分组下面某个材料的颜色
    sceneRef.current?.getObjectByName('battery_1_8')?.traverse((child) => {
      if ((child as Mesh).isMesh) {
        const name = child.name;
        // if (name.includes('battery_4')) {
        //   console.log({name, child, parent: child.parent?.name,})
        // }
        // TODO
        if (name.includes('battery_4')) {
  ...
  const groupClone = useCallback(async (group: Group) => {
    for (let i = 0; i < 21; i++) {
      ...
      const text = getText(
        `${(Math.random() * 100).toFixed(1)}% ${(Math.random() * 100).toFixed(
          2,
        )}`,
        1.2,
      );
      if (text) {
        groupCloned1?.add(text);
      }
      // groupCloned1.position.z = -20 * i;
      const groupIndex = Math.floor(i / 7);
      const groupRowIndex = i % 7;
      groupCloned1.position.y = 5 * groupRowIndex;
      groupCloned1.position.x = 10 * groupIndex;
  ...
  const onMouseClick = useCallback((e) => {
    ...
    if (intersects.length > 0) {
      ...
      if (!selectedObj.name.includes('battery')) {
        return;
      }
      ...
      const getParentGroupName = (obj: THREE.Object3D<THREE.Event>): string => {
        const parent = obj.parent;
        if (parent?.name.includes('battery_1_')) {
          return parent.name;
        }
        if (!parent) {
          return '';
        }
        return getParentGroupName(parent);
      };
      // TODO
      // if (selectedObj.name.includes('battery_4')) {
      //   (selectedObj as Mesh).material.color.set('red');
      // }
      const parentGroupName = getParentGroupName(selectedObj);
      const parentGroup = sceneRef.current?.getObjectByName(parentGroupName);
      console.log({
        parentGroup,
      ...
      parentGroup?.traverse((child) => {
        if ((child as Mesh).isMesh) {
          ...
          if (name.includes('battery_4')) {
            (child as Mesh).material.color.set('red');
          }

          if (name.includes('battery_1_')) {
            ...
            box.getSize(size);
            console.log({ size });
            // const size = fileSizeMapRef.current.battery
            const { x, y, z } = size;
            const geometry = new BoxGeometry(
              ...[x, y, z].map((val) => val * 20 + 10),
            );
            ...
            //   opacity: 0.01,
            // })
            const errorBoxCube = new Mesh(geometry, material);
            console.log({
              errorBoxCube,
              x: x * 20,
              y: y * 20,
            ...
            // errorBoxCube.position.x = x * 10 + 2.5
            // errorBoxCube.position.y = y * 10 + 4.492
            // errorBoxCube.position.z = - z * 10 + 16.613
            errorBoxCube.position.x = x * 10 + 2.5;
            errorBoxCube.position.y = y * 10 + 4.492;
            errorBoxCube.position.z = -z * 10 + 16.613;
            parentGroup.add(errorBoxCube);

            const cubeEdges = new EdgesGeometry(geometry, 1);
            const edgesMtl = new LineBasicMaterial({
              color: 0xff0000,
            });
            const cubeLine = new LineSegments(cubeEdges, edgesMtl);
            cubeLine.position.x = x * 10 + 2.5;
            cubeLine.position.y = y * 10 + 4.492;
            cubeLine.position.z = -z * 10 + 16.613;
            parentGroup.add(cubeLine);
            // LineBasicMaterial,
            // LineSegments,
            ...
            console.log({
              ...
              child,
              errorBoxCube,
              fileSizeMapRef: fileSizeMapRef.current,
            });
          }
        }
      });
  ...
  const init = useCallback(async () => {
    ...
    const inverterGltf = await loadFile(
      'models/battery_yu/battery.gltf',
      'battery',
    );
    inverterGltf.scene.position.x = 0;
    inverterGltf.scene.position.y = 0;
    inverterGltf.scene.position.z = -1;
    inverterGltf.scene.scale.set(0.05, 0.05, 0.05);
